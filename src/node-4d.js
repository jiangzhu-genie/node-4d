...
DbConnection.prototype.readPacket = function () {
  ...
  const string = this.buffer.toString('ascii', 0, 32);
  let matches = [];
...
DbConnection.prototype.createCommand = function (type) {
  const command = new DbCommand(this.nextCommandID++, type);
  return command;
};
...
DbConnection.prototype.connect = function (callback) {
  ...
  this.socket.connect(this.port, this.host, function () {
    ...
    const command = self.createCommand('LOGIN');
...
DbConnection.prototype.end = function () {
  ...
  const logoutCommand = this.createCommand('LOGOUT');
  ...
  const quitCommand = this.createCommand('QUIT');
...
DbConnection.prototype.query = function (sql, params, callback) {
  ...
  const command = this.createCommand('EXECUTE-STATEMENT');
...
DbConnection.prototype.fetch = function (result, callback) {
  ...
  const firstRow = result.rows.length + 1;
  const lastRow = result.rows.length + this.fetchLimit;

  const command = this.createCommand('FETCH-RESULT');
...
DbCommand.prototype.parseHeaders = function (buffer) {
  let headers = {};
  let offset = buffer.indexOf(kCRLF + kCRLF);
  const packet = buffer.toString('utf8', 0, offset);
  ...
  if (packet.match(/^\d+ \w+$/m) && offset != 0) {
    ...
    for (let i = 1; i < lines.length; i++) {
      pos = lines[i].indexOf(':');
      if (pos == -1) { continue; }
      let key = lines[i].substr(0, pos).trim();
      let value = lines[i].substr(pos + 1).trim();
      ...
      if (key.startsWith('Stack-Error')) {
        value = value.replace(/(\w+ \d+ \d+ )(.+)/, (fullMatch, part1, part2) => part1 + base64Decode(part2));
      }
...
DbCommand.prototype.setResponseHeaders = function (headers) {
  ...
  if (headers.status == 'ERROR') {
    ...
    if (headers.StackError1) { this.errors.message += `\n${headers.StackError1}`; }
    if (headers.StackError2) { this.errors.message += `\n${headers.StackError2}`; }
    if (headers.StackError3) { this.errors.message += `\n${headers.StackError3}`; }
    return;
  }
  ...
  if (headers.ResultType == 'Result-Set' || headers.ResultType == 'Update-Count') {
    ...
    result.fields = [];
    result.rows = [];
    for (let i = 0; i < headers.ColumnTypes.length; i++) {
      result.fields.push({
...
DbCommand.prototype.parseRows = function (buffer) {
  let offset = 0;
  const result = [];
  let offsetLastRecord = 0;

  // Fill an array with the column names and types
  const names = this.result.headers.ColumnAliases.slice();
  const types = this.result.headers.ColumnTypes.slice();
  ...
  while (offset < buffer.length) {
    ...
    try {
      for (let colNr = 0; colNr < types.length; colNr++) {
        if (offset >= buffer.length) {
          break;
        }

        const name = names[colNr];
        const type = types[colNr];
        let length = 0;
        var value;
        var error;
        const status = buffer.toString('ascii', offset, offset + 1);
        offset++;
...
DbConnectionPool.prototype.query = function (sql, params, callback) {
  ...
  this.getConnection((errors, connection) => {
    if (errors) {
...
function splitList(value) {
  ...
  if (value.indexOf('[') != -1) {
    array = value.split(/\[(\w+)\]\s*/).filter((value) => value.length > 0);
  } else {
    array = value.split(' ');
  }
...
function prepareSQL(sql, params) {
  if (typeof params === 'object') {
    ...
    sql = sql.replace(/\$\w+/gi, (key) => {
      key = key.substr(1, key.length);
      let value = params[key];
      // Wrap the string with single quotes if needed
      if (typeof value === 'string') { value = `'${value.replace(/'/g, "\'")}'`; } else if (typeof value === 'undefined') { throw new Error(`Key '${key}' is undefined in sql '${sql}'`); }
      return value;
    });
  }
